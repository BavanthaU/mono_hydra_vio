...
bool FeatureDetectorParams::parseYAML(const std::string& filepath) {
  ...
  switch (feature_detector_type) {
    ...
    case VIO::to_underlying(FeatureDetectorType::SUPERPOINT): {  // Add SuperPoint case
      feature_detector_type_ = FeatureDetectorType::SUPERPOINT;

      // Load SuperPoint model path
      yaml_parser.getYamlParam("superpoint_path", &superpoint_model_path_);
      if (superpoint_model_path_.empty()) {
          LOG(FATAL) << "SuperPoint model path (superpoint_path) must be specified in the YAML file.";
      }
      LOG(INFO) << "Loaded SuperPoint model path: " << superpoint_model_path_;

      // Load SuperPoint score threshold
      yaml_parser.getYamlParam("superpoint_score_threshold", &superpoint_score_threshold_);
      if (superpoint_score_threshold_ <= 0.0) {
          LOG(WARNING) << "SuperPoint score threshold (superpoint_score_threshold) not specified in the YAML file. Using default value: " 
                      << superpoint_score_threshold_;
      } else {
          LOG(INFO) << "Loaded SuperPoint score threshold: " << superpoint_score_threshold_;
      }
      break;
    }
